...
function resolveHistoryId(groupId, senderId, no) {
  const ids = historyCache.get(cacheKey(groupId, senderId)) || [];
...
async function riwayat(ctx) {
  ...
  if (!filter) return 'Format riwayat salah. Contoh: riwayat | riwayat 50 | riwayat hari ini | riwayat 2026-02-18 | riwayat 2026-02-01 2026-02-18';
  ...
  if (filter.type === 'latest') {
    ...
    `).all(ctx.groupId, filter.start, filter.end);
  }
...
async function detail(ctx) {
  ...
  if (!no) return null;
  const id = resolveHistoryId(ctx.groupId, ctx.senderId, no);
  ...
  return [
    ...
    `Group: ${tx.group_id}`,
    `Tipe: ${tx.type}`,
    `Nominal: ${tx.type === 'income' ? '+' : '-'}${formatRupiah(tx.amount)}`,
    `Catatan: ${tx.note}`,
    `Pengirim: ${tx.sender_name || '-'} (${tx.sender_id})`,
    `Dibuat: ${formatWib(tx.created_at)}`,
    `Edited: ${tx.edited_at ? formatWib(tx.edited_at) : '-'}`,
    `Deleted: ${tx.deleted_at ? formatWib(tx.deleted_at) : '-'}`
  ].join('\n');
}
...
async function edit(ctx, isAdmin) {
  ...
  const id = resolveHistoryId(ctx.groupId, ctx.senderId, parsed.no);
  if (!id) return 'Nomor tidak ditemukan. Jalankan riwayat dulu.';
  ...
  if (!isAdmin && tx.sender_id !== ctx.senderId) return '⛔ Hanya admin grup atau pembuat transaksi yang boleh edit.';
  ...
  updateTransaction({
    ...
    amount: parsed.amount,
    note: parsed.note,
    edited_at: DateTime.now().setZone(TIMEZONE).toISO()
  });
...
async function remove(ctx, isAdmin) {
  ...
  const tx = db.prepare('SELECT * FROM transactions WHERE id=? AND deleted_at IS NULL').get(id);
  if (!tx) return 'Transaksi tidak ditemukan atau sudah dihapus.';
  if (!isAdmin && tx.sender_id !== ctx.senderId) return '⛔ Hanya admin grup atau pembuat transaksi yang boleh hapus.';
...
async function saldo(ctx) {
  ...
  const where = range
    ? `group_id=? AND deleted_at IS NULL AND datetime(created_at)>=datetime(?) AND datetime(created_at)<datetime(?)`
...
module.exports = { recordTransaction, riwayat, detail, edit, remove, saldo };
